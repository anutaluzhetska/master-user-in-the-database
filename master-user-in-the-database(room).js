...
// Спеціальна функція для адміна - видалити будь-кого без пароля
async function adminDeleteUser(targetEmail) {
  const res = await pool.query(
    "DELETE FROM kotik WHERE email = $1 RETURNING *",
    [targetEmail.toLowerCase()],
  );
  if (res.rowCount > 0) {
    console.log(`[ADMIN] User ${targetEmail} FORCE REMOVED. `);
  } else {
...
async function showStats() {
  const res = await pool.query(
    "SELECT COUNT(*) as total, MAX(created_at) as last_reg FROM kotik",
  );
  console.log(`
        DATABASE STATS:
------------------
Total users: ${res.rows[0].total}
Last registration: ${res.rows[0].last_reg}
------------------`);
}
  ...
  try {
    ...
    switch (command) {
      ...
      case "update":
        ...
        break;
      case "admin-delete":
        const masterKey = await rl.question("Enter MASTER KEY: ");
        if (masterKey === process.env.MASTER_PASSWORD) {
          const target = await rl.question("email to delete: ");
          await adminDeleteUser(target);
        } else {
          console.log("INVALID MASTER KEY. Access denied.");
        }
        break;
